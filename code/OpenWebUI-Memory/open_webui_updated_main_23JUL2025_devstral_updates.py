...
async def store_interaction(user_input, response_content, conversation_id):
    ...
    text_embeddings = await embed_text(interaction_chunks)
    ...
    client = await AsyncHttpClient(host='chromadb', port=8000)
    ...
    context = "general"  # Update if needed for different contexts
    interaction_id = str(uuid.uuid4())
    metadata = {
        "response": response_content,
        "timestamp": time.time(),
        "context": context,
        "user_id": user.id,  # Ensure this is passed from the request
    ...
    log.info(f"Stored {len(interaction_chunks)} text chunks with interaction_id: {interaction_id}")
    ...
    if "messages" in form_data and isinstance(form_data["messages"], list):
        for message in form_data["messages"]:
            if "role" in message and message["role"] == "user":
                message["timestamp"] = int(time.time())  # Add epoch time as a timestamp
